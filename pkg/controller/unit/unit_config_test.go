...
var _ = Describe("UnitConfig Reconciler", func() {
	...
	BeforeEach(func() {
		...
		// Create test unit
		unit = &upmiov1alpha2.Unit{ObjectMeta: metav1.ObjectMeta{Name: unitName, Namespace: "default"}, Spec: upmiov1alpha2.UnitSpec{Startup: true, ConfigTemplateName: "test-config-template", ConfigValueName: "test-config-value", Template: upmiov1alpha2.UnitPodTemplateSpec{Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "main-container", Image: "nginx:latest"}, {Name: "unit-agent", Image: "unit-agent:latest"}}}}}}
